...
`v0.8.2` — alpha, Windows-first. Playback, library scanning, playlists, favorites, lyrics, search across SoundCloud and YouTube Music, browsing and caching for both, Discord presence, a floating mini player, right-click menus throughout, two player bar layouts, crossfade, a spectrum visualiser and a built-in updater all work. The release pipeline currently ships a Windows NSIS installer only; the codebase itself has no Windows-specific dependencies beyond the Discord IPC pipe path and taskbar progress.
